 ...
 * @property {string}    action     - The current state as initially derived from the request.  This is mutible, whereas resquest.action should always maintain its original value.  Middleware can safely change this value.
 ...
 * @property {BaseArray} errors     - If errors are thrown during handler execution, they are pushed to this array.
 * @property {boolean}   hasErrors  - If one or more errors occur during handler execution, this will be set to true.
 ...
 * @property {boolean}   shouldAbort      - When set to true, execution is prevented.  This is best used for things like middleware that verifies request sources.
...
class Context extends BaseObject {
  ...
  */
  ...
  constructor(requestObject){
    ...
    this.set('storage',    {});
    this.set('temp',       {});
    this.set('errors',     new BaseArray());
    this.set('shouldAbort',      false);
    this.loadRequest();
  ...
  */
  ...
  loadRequest(){
    ...
    this.set('response.sessionId', request.get('sessionId'));
  }
  /**
   * Prevents further execution after the current action finishes.
   */
  abort(){
    this.set('shouldAbort', true);
  }
  /**
