...
class Skill extends BaseObject {
  ...
  use(...middlewares){
    ...
    middleware.concatObject(middlewares.map(m => {
      return (typeof m === 'function') ? new m() : m;
    }));
    return this;
  }
  /**
  ...
  */
  async handleAction(context, name){
   ...
   * @method
   * @param   {object|Context} body    - A request body or an instance of Context within which the application code will run.
   * @param   {object={}}      headers - Headers and other extra data included with the request.
  ...
  */
  async perform(body, headers={}){
    ...
    const context    = Context.from(body),
          ...
          execute    = async (actionName) => {
            if(context.get('shouldAbort')) return;
            await this.handleAction(context, actionName);
    ...
    let ready = false;
    try {
      await Promise.all(middleware.map(m => (m.before || function(){}).apply(m, [context, headers])));
      ready = true;
    } catch(err) {
    ...
    try {
      ...
      if(ready) await Promise.all(middleware.map(m => (m.after || function(){}).apply(m, [context, headers])));
    } catch(err) {
      await handleError(err);
    }
   ...
   * This will work for Lambda functions that are invoked by either the Alexa Skills Kit or API Gateay triggers.
   * @method
   * @returns {Promise}
   */
  lambda(){
    return async (event, context, callback) => {
      ...
      const isAPIGateway = event.requestContext ? true : false;
      let body, headers;
      if(isAPIGateway){
        body    = event.body;
      ...
      return this.perform((isAPIGateway ? event.body : event), headers)
        .then(res  => { 
          let body;
          if(isAPIGateway) {
            body = {
              statusCode: res.statusCode,
              body:       stringify(res.json)
            };
          } else {
            body = res.json;
          }
  ...
  whisk(){
    return async (msg) => {
      ...
      return this.perform(msg, msg.__ow_headers)
                 .then(res => {
                   return {
                     statusCode: res.statusCode,
