...
class GoogleRequest extends Request {
  get isNew(){
    return this.get('originalRequest.conversation.type') === 'NEW';
  }
  get userId(){
  ...
  get attributes(){
    ...
    const conversationToken = this.get('originalRequest.conversation.conversation_token', '{}');
    try{
      return parse(conversationToken)['ATTRIBUTES'] || {};
    } catch(err) {
      return {};
    }
  }
  get state(){
    const conversationToken = this.get('originalRequest.conversation.conversation_token', '{}');
    try {
      return (parse(conversationToken) || {})['STATE'];
    } catch(err) {
      return '';
    }
  }
